...
class ElasticsearchRulesTest {
  ...
  @Test void plainNamesUnchangedByEscaping() {
    assertThat(ElasticsearchRules.escapeSpecialSymbols("city"), is("city"));
    assertThat(ElasticsearchRules.escapeSpecialSymbols("b.a"), is("b.a"));
  }
  ...
  @Test void quotesAndBackslashesEscaped() {
    ...
    assertThat(ElasticsearchRules.escapeSpecialSymbols("a\"b"), is("a\\\"b"));
    assertThat(ElasticsearchRules.escapeSpecialSymbols("a\\b"), is("a\\\\b"));
    assertThat(ElasticsearchRules.escapeSpecialSymbols("a\\"), is("a\\\\"));
    // Order matters: escape backslash first, then double quote, so
    // "a\"b" doesn't become "a\\\"b" with a lost closing quote.
    assertThat(ElasticsearchRules.escapeSpecialSymbols("a\\\"b"),
        is("a\\\\\\\"b"));
  }
  ...
  @Test void namesWithJsonMetacharactersStayASingleSourceField() throws Exception {
    ...
    final String json = "{\"_source\" : ["
        + ElasticsearchRules.quote(
            ElasticsearchRules.escapeSpecialSymbols(awkward)) + "]}";
  ...
  @Test void scriptFieldEntryStaysWellFormedJsonForAwkwardName() throws Exception {
    ...
    final String entry =
        ElasticsearchRules.quote(ElasticsearchRules.escapeSpecialSymbols(awkward))
            + ":{\"script\": "
            + ElasticsearchRules.quote(
                ElasticsearchRules.escapeSpecialSymbols(
                    ElasticsearchRules.scriptedFieldAccess("params._source",
                        awkward)))
            + "}";
